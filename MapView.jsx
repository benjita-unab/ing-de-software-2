...
// ─────────────────────────────────────────────────────────────────────────────
// Visor de mapa con Google Maps JavaScript API.
// onFocusMap(alert) centra el mapa en el camión afectado (CA-2).
//
// CONFIGURACIÓN REQUERIDA:
//   1. Habilitar "Maps JavaScript API" en Google Cloud Console.
//   2. Agregar en tu .env:
...
export default function MapView({ alerts, focusedAlert }) {
  ...
  useEffect(() => {
    ...
    // Eliminar marcadores de alertas que ya no están
  ...
  // ── Centrar mapa en alerta enfocada (CA-2: enlace directo a ubicación) ──
  useEffect(() => {
    ...
    // Rebote temporal para destacar el marcador
    ...
    return (
      <div style={errorContainerStyle}>
        ...
        <p style={{ color: "#888", fontSize: "12px", margin: 0, maxWidth: "260px", textAlign: "center" }}>
          Agrega <code style={{ color: "#ffd54f" }}>REACT_APP_GOOGLE_MAPS_API_KEY</code> en tu archivo .env y recarga la página.
        </p>
      </div>
    );
  }
  ...
  return (
    <div style={{ position: "relative", width: "100%", height: "100%" }}>
      ...
      {/* Panel flotante: resumen de activos en mapa */}
      {mapLoaded && (
        <MapLegend alerts={alerts} />
      )}

      {/* Indicador de cámara enfocada */}
      {focusedAlert && (
...
// Estilo oscuro para el mapa (coherente con el dashboard)
